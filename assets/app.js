...
selectCallback = function(variant, selector) {
  ...
  //Image Variant feature
   if (variant && variant.featured_image && $product.is(":visible")) {
    var $sliders = $('.product_gallery_custom', $product);
     if($('.product-single__photo #ProductPhotoImg-product-template') == 1)
     {
       if(variant.featured_image.src)
         	$('.product-single__photo #ProductPhotoImg-product-template').attr('src',variant.featured_image.src);
     }
  ...
  if (variant) {
	...
	$('.ar-quicklook-overlay').attr('data-shopify-3d-variant-id',variant.id);
    $('.sku span', $product).text(variant.sku);

    
      $(document).on('change', '.single-option-selector', function () {
        var radioButtonPoster = $productForm.find('.swatch[data-option-index="2"] :radio[value="Poster"]');
        var radioButtonPosterAvailable = $productForm.find('.swatch[data-option-index="2"] :radio[value="Poster"]+.swatch-element').css('display');
        var selectorOption = $(this).attr('id').split('product-template-option-');
        var selectorTitleText = $('.swatch[data-option-index="'+selectorOption[1]+'"]').find('.option_title').text().toLowerCase();
        if(selectorTitleText != 'frame'){
          if(radioButtonPosterAvailable == 'inline-block'){
            radioButtonPoster.click();
            var length=variant.options.length - 1;
          }
        }
      });
      var radioButtonPoster = $productForm.find('.swatch[data-option-index="2"] :radio[value="Poster"]');
...
var frameProperties = function(variant){
    var currentVariantTitle = variant.title,
        framePropertiesValue = '',
        frameVariantId;
    if(currentVariantTitle.indexOf('Gallery Wrap + Floating Frame') > -1){
    	currentVariantTitle = currentVariantTitle.replace('Gallery Wrap + Floating Frame','Gallery Wrap');
        framePropertiesValue = framePropertiesValue+'simpdata_id_'+variant.id;
        frameVariantId = $('.multi_select').find('option[data-title="'+currentVariantTitle+'"]').attr('value');
        if(frameVariantId != '' && frameVariantId != undefined){
        	framePropertiesValue = framePropertiesValue+',simp_id_'+frameVariantId;
        }
    }else if(currentVariantTitle.indexOf('Gallery Wrap') > -1){
    	currentVariantTitle = currentVariantTitle.replace('Gallery Wrap','Gallery Wrap + Floating Frame');
        framePropertiesValue = framePropertiesValue+'simp_id_'+variant.id;
        frameVariantId = $('.multi_select').find('option[data-title="'+currentVariantTitle+'"]').attr('value');
        if(frameVariantId != '' && frameVariantId != undefined){
        	framePropertiesValue = framePropertiesValue+',simpdata_id_'+frameVariantId;
        }
    }
    $('.frame_properties').val(framePropertiesValue);
}
...
var productPage = {
  init: function(){
    ...
    var autoplay = $('.product_gallery_custom').data('autoplay');

    setTimeout(function(){
     imageFunctions.linkGalleryAndCarousel($('.js-product-page-gallery .product_gallery_custom'), $('.js-product-page-gallery .product_gallery_nav'));
    }, 1000);
    imageFunctions.zoom('.product_gallery_custom', false);
    ...
    if ($('.product_gallery_custom').hasClass('single-image')){
      draggable = false;
...
*/
    $('.product_gallery_custom').flickity({
      "wrapAround": "true",
      "cellAlign": "left",
      "draggable": draggable,
      "contain": true,
	...
	if($('.product_gallery_nav .gallery-cell').length > 5){ 
      $('.product_gallery_nav').flickity({
        asNavFor: '.product_gallery_custom',
        draggable: false,
...
//   });
  ...
  function ajaxUpdateCart(key, lineID, quantity, parent) {
    $.ajax({
      url: '/cart/change.js',
      dataType: 'json',
      cache: false,
      type: 'post',
      data: { quantity: quantity, id: key },
      success: function (data) {
        refreshCart(data);
        ...
        if (typeof updatedItem === "undefined"){
          ...
          var updated_quantity = updatedItem.quantity;
        }
        ...
        if(quantity > 0 && quantity != updated_quantity) {
          ...
          parent.find("input[data-item-key='" + key + "']").parent().after(warning);
          parent.find("input[data-item-key='" + key + "']").val(updated_quantity);
  ...
  function refreshCart(cart) {
    ...
    if(savedBy) {
      cart.items = cart.items.filter(i => i !== savedBy);
      cart.item_count--;
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        ...
        cart_items_html += '<div class="left product-quantity-box 123">' +
            '<span class="ss-icon product-minus js-change-quantity" data-func="minus"><span class="icon-minus"></span></span>' +
            '<input type="number" min="0" class="quantity" name="updates[]" id="updates_' + item.id + '" value="' + item.quantity +'" data-line-id="' + line_id +'" data-item-key="'+ item.key +'" />' +
  ...
  $('body').on("change", "#cart_form input.quantity", function() {
    ajaxUpdateCart($(this).data('item-key'), $(this).data('line-id'), $(this).val(), $(this).parents('#cart_form'));
  });

  $('body').on("change", ".cart_content input.quantity", function() {
    ajaxUpdateCart($(this).data('item-key'), $(this).data('line-id'), $(this).val(), $(this).parents('.cart_content'));
  });
...
$(document)
  ...
  $("a.scroll_to_product").click(function(e) {
	e.preventDefault();
